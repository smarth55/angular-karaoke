...
function assertSameDimensions({width: widthA, height: heightA}: HasDimensions, {width: widthB, height: heightB}: HasDimensions, nameA: string, nameB: string) {
...
function drawWithCompositing(ctx: CanvasRenderingContext2D, image: HTMLCanvasElement|ImageType, compositOperation: string) {
...
function drawAndBlurImageOnOffScreenCanvas(image: ImageType, blurAmount: number, offscreenCanvasName: string): HTMLCanvasElement {
 ...
 */
export function toColoredPartImageData(partSegmentation: PartSegmentation, partColors: Array<[number, number, number]>): ImageData {
 ...
 * to false.
 */
export function drawMask(canvas: HTMLCanvasElement, image: ImageType, maskImage: ImageData, maskOpacity = 0.7, maskBlurAmount = 0, flipHorizontal = false) {
 ...
 */
export function drawPixelatedMask(canvas: HTMLCanvasElement, image: ImageType, maskImage: ImageData, maskOpacity = 0.7, maskBlurAmount = 0, flipHorizontal = false, pixelCellWidth = 10.0) {
  assertSameDimensions(image, maskImage, 'image', 'mask');
 ...
 * to false.
 */
export function drawBokehEffect(canvas: HTMLCanvasElement, image: ImageType, personSegmentation: PersonSegmentation, backgroundBlurAmount = 3, edgeBlurAmount = 3, flipHorizontal = false) {
  assertSameDimensions(image, personSegmentation, 'image', 'segmentation');

  const blurredImage = drawAndBlurImageOnOffScreenCanvas(image, backgroundBlurAmount, CANVAS_NAMES.blurred);
